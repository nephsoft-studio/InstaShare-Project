<?php
	...
	function instashare_above_header() {
		...
		$hdr_top_icon_menu 			= get_theme_mod( 'hdr_top_icon_menu',instashare_get_icon_menu_default());
		...
		if($hs_hdr_icon_menu =='1' || $hide_show_language_details =='1' || $hide_show_settings_details =='1' || $hide_show_help_details =='1'):
		?>
												...
												<ul id="menu-footer-menu" class="menu">
													<?php
														...
														foreach ( $hdr_top_icon_menu as $icon_menu_item ) {
															$title = ! empty( $icon_menu_item->title ) ? apply_filters( 'instashare_translate_single_string', $icon_menu_item->title, 'Header section' ) : '';
															$icon = ! empty( $icon_menu_item->icon_value ) ? apply_filters( 'instashare_translate_single_string', $icon_menu_item->icon_value, 'Header section' ) : '';
															$link = ! empty( $icon_menu_item->link ) ? apply_filters( 'instashare_translate_single_string', $icon_menu_item->link, 'Header section' ) : '';
													?>
