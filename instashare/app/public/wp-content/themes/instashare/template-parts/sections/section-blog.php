<?php  
	$instashare_blog_hs 			= get_theme_mod('blog_hs','1');
	$instashare_blog_title 		= get_theme_mod('blog_title');
	$instashare_blog_desc			= get_theme_mod('blog_description');  
	$instashare_blog_display_num	= get_theme_mod('blog_display_num','3'); 
	if($instashare_blog_hs=='1'){
?>	
<section id="post-section" class="post-section av-py-default blog-home shapes-section">
	<div class="av-container">
		<?php if(!empty($instashare_blog_title)  || !empty($instashare_blog_desc)): ?>
			<div class="av-columns-area">
				<div class="av-column-12">
					<div class="heading-default text-center wow fadeInUp">
						<?php if(!empty($instashare_blog_title)): ?>
							<h3><?php echo wp_kses_post($instashare_blog_title); ?></h3>
							<span class="separator"><span><span></span></span></span>
						<?php endif; ?>
						<?php if(!empty($instashare_blog_desc)): ?>
							<p><?php echo wp_kses_post($instashare_blog_desc); ?></p>
						<?php endif; ?>	
					</div>
				</div>
			</div>
		...
		<div class="av-columns-area">
			<div class="av-column-12">
				<div class="post-carousel owl-carousel owl-theme">
					<?php 
					$instashare_blog_args = array( 'post_type' => 'post', 'posts_per_page' => $instashare_blog_display_num,'post__not_in'=>get_option("sticky_posts")) ; 	
					
				$instashare_wp_query = new WP_Query($instashare_blog_args);
					if($instashare_wp_query)
					{	
					 $post_count=0;
					while($instashare_wp_query->have_posts()):$instashare_wp_query->the_post();
									...
									<span class="post-count">
										<?php
											if($post_count<9):
												...
												echo sprintf(esc_html('0 %s','instashare'), $post_count);
											else:		
